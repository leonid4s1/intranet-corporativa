      ...
      <section class="card">
        <div class="card-head">
          ...
          <div class="head-actions">
            <button class="btn primary" @click="openHolidayModal()">
              Agregar Festivo
            </button>
        ...
        <div class="modal">
          ...
          <div class="modal-actions">
            ...
            <button
              ...
              :disabled="
                reject.loading ||
                reject.text.trim().length < 3 ||
                reject.text.trim().length > 500
              "
              :title="
                reject.text.trim().length < 3
                  ? 'Mínimo 3 caracteres'
          ...
          <div class="form-grid">
            <label>
              ...
              <input
                ...
                autocomplete="off"
              />
              <small v-if="holidayErrors.name" class="error">{{
                holidayErrors.name
              }}</small>
            </label>
            ...
            <label>
              ...
              />
              <small v-if="holidayErrors.date" class="error">{{
                holidayErrors.date
              }}</small>
            </label>
...
async function confirmReject() {
  ...
  try {
    ...
    isActionLoading.value = id;
    await vacationService.updateRequestStatus({
      id,
      status: 'rejected',
      rejectReason: text,
    });
  ...
  try {
    ...
    if (holidayModal.value.mode === 'create') {
      await holidayService.createHoliday({
        ...
        description:
          holidayForm.value.description?.trim() || undefined,
...
async function deleteHoliday(h: Holiday) {
  const ok = window.confirm(
    `¿Eliminar festivo "${h.name}" del ${formatDate(h.date)}?`,
  );
  if (!ok) return;
